...
import net.ion.nradon.handler.ReconnectingWebSocketHandler;
...
public class WebSocketClient implements WebSocket {
            ...
            public WebSocketClient call() throws Exception {
                ...
                bootstrap.setPipelineFactory(new ChannelPipelineFactory() {
                    public ChannelPipeline getPipeline() throws Exception {
                        ...
                        if (ssl) {
                            if (sslFactory == null) {
                                throw new RadonException("You need to call setupSsl first");
                            }
    ...
    private HttpRequest createNettyHttpRequest() {
        ...
        if(!first){
            request.setHeader(net.ion.nradon.HttpRequest.COOKIE_HEADER, builder.toString());
        }
        ...
        FutureTask<WebSocketClient> future = new FutureTask<WebSocketClient>(new Callable<WebSocketClient>() {
            public WebSocketClient call() throws Exception {
                try {
                    ...
                    exceptionHandler.uncaughtException(Thread.currentThread(), RadonException.fromException(e, channel));
                }
                return WebSocketClient.this;
            }
        });
        ...
        private void verifySecWebSocketAccept(String webSocketAccept) {
            if (webSocketAccept != null) {
                ...
                if (!webSocketAccept.equals(expectedKey)) {
                    throw new RadonException("Sec-WebSocket-Accept header from server didn't match expected value of " + expectedKey);
                }
            } else {
                throw new RadonException("Expected Sec-WebSocket-Accept header from server");
            }
        }
