 ...
 * RFC 2965 - <a href="http://www.ietf.org/rfc/rfc2965.txt"> <i>http://www.ietf.org/rfc/rfc2965.txt</i></a> </blockquote>
 * 
 * <p>
...
public final class HttpCookie implements Cloneable {
	...
	private static Logger log = Logger.getLogger(HttpCookie.class.getCanonicalName());
	 ...
	 * 
	 * <p>
	 * The value can be anything the server chooses to send. Its value is probably of interest only to the server. The cookie's value can be changed after creation with the <code>setValue</code> method.
	 * 
	 * <p>
	 * By default, cookies are created according to the RFC 2965 cookie specification. The version can be changed with the <code>setVersion</code> method.
	 * 
	 * 
	 * @param name
	 ...
	 * 
	 * @param header
	 *            a <tt>String</tt> specifying the set-cookie header. The header should start with "set-cookie", or "set-cookie2" token; or it should have no leading token at all.
	 ...
	 * 
	 * @return <tt>true</tt> to indicate this http cookie has expired; otherwise, <tt>false</tt>
	 */
	...
	/**
	 * Returns the comment url describing the purpose of this cookie, or <code>null</code> if the cookie has no comment url.
	 * 
	 * @return a <code>String</code> containing the comment url, or <code>null</code> if none
	 * 
	 * @see #setCommentURL
	 * 
	 */
	 ...
	 * 
	 * @param ports
	 *            a <tt>String</tt> specify the port list, which is comma seperated series of digits
	 * @see #getPortlist
	 */
	 ...
	 * 
	 * <p>
	 * A positive value indicates that the cookie will expire after that many seconds have passed. Note that the value is the <i>maximum</i> age when the cookie will expire, not the cookie's current age.
	 * 
	 * <p>
	...
	/**
	 * Returns the maximum age of the cookie, specified in seconds. By default, <code>-1</code> indicating the cookie will persist until browser shutdown.
	 * 
	 * 
	 ...
	 * 
	 * <p>
	 * The cookie is visible to all the pages in the directory you specify, and all the pages in that directory's subdirectories. A cookie's path must include the servlet that set the cookie, for example, <i>/catalog</i>, which makes the cookie visible to all directories on the server under <i>/catalog</i>.
	 * 
	 * <p>
	 ...
	 * 
	 * @param flag
	 *            if <code>true</code>, sends the cookie from the browser to the server using only when using a secure protocol; if <code>false</code>, sent on any protocol
	 * 
	 * @see #getSecure
	 * 
	 */
	 ...
	 * 
	 * @return <code>true</code> if the browser can use any standard protocol; otherwise, <code>false</code>
	 * 
	 * @see #setSecure
	 * 
	 */
	 ...
	 * 
	 * 
	 * @return 0 if the cookie complies with the original Netscape specification; 1 if the cookie complies with RFC 2965/2109
	 * 
	 * @see #setVersion
	 * 
	 */
	 ...
	 * 
	 * <p>
	 * This concept is described in the cookie specification. To understand the concept, some terminologies need to be defined first: <blockquote> effective host name = hostname if host name contains dot<br>
	 ...
	 * <li>A is a HDN string and has the form NB, where N is a non-empty name string, B has the form .B', and B' is a HDN string. (So, x.y.com domain-matches .Y.com but not Y.com.)</li>
	 * </ul>
	 * </blockquote>
	 * 
	 * <p>
	 ...
	 * <li>The request-host is a HDN (not IP address) and has the form HD, where D is the value of the Domain attribute, and H is a string that contains one or more dots.</li>
	 * </ul>
	 * </blockquote>
	 * 
	 * <p>
	 ...
	 * <li>A Set-Cookie2 with Domain=ajax.com will be accepted, and the value for Domain will be taken to be .ajax.com, because a dot gets prepended to the value.</li>
	 * <li>A Set-Cookie2 from request-host example for Domain=.local will be accepted, because the effective host name for the request- host is example.local, and example.local domain-matches .local.</li>
	 * </ul>
	 * </blockquote>
	 * 
	 * @param domain
	 ...
	 * 
	 * <p>
	 * The result is <tt>true</tt> only if two cookies come from same domain (case-insensitive), have same name (case-insensitive), and have same path (case-sensitive).
	 * 
	...
	public boolean equals(Object obj) {
		...
		return equalsIgnoreCase(getName(), other.getName()) && equalsIgnoreCase(getDomain(), other.getDomain()) && equals(getPath(), other.getPath());
	}

	/**
	 * Return hash code of this http cookie. The result is the sum of hash code value of three significant components of this cookie: name, domain, and path. That is, the hash code is the value of the expression: <blockquote> getName().toLowerCase().hashCode()<br>
	 ...
	 * 
	 * @return <code>true</code> if the <code>String</code> is a token; <code>false</code> if it is not
	 */
	 ...
	 * 
	 * @return <tt>true</tt> if the name is reserved by cookie specification, <tt>false</tt> if it is not
	 */
	private static boolean isReserved(String name) {
		if (name.equalsIgnoreCase("Comment") || name.equalsIgnoreCase("CommentURL") // rfc2965 only
				...
				|| name.equalsIgnoreCase("Max-Age") || name.equalsIgnoreCase("Path") || name.equalsIgnoreCase("Port") // rfc2965 only
	 ...
	 * 
	 * @throws IllegalArgumentException if header string violates the cookie specification
	 */
	...
	private static void assignAttribute(HttpCookie cookie, String attrName, String attrValue) {
		...
		if (assignor != null) {
			...
			log.warning("Illegal cookie attribute [" + attrName + " : " +  attrValue + "]");
		}
	}
	...
	/*
	 * @param dateString a date string in format of "EEE',' dd-MMM-yyyy HH:mm:ss 'GMT'", which defined in Netscape cookie spec
	 * 
	 * @return delta seconds between this cookie's creation time and the time specified by dateString
	 */
	...
	/*
	 * Split cookie header string according to rfc 2965: 1) split where it is a comma; 2) but not the comma surrounding by double-quotes, which is the comma inside port list or embeded URIs.
	 * 
