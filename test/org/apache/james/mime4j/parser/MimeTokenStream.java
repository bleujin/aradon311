 ...
 * </pre>
 * <p>
 * Instances of {@link MimeTokenStream} are reusable: Invoking the method {@link #parse(InputStream)} resets the token streams internal state. However, they are definitely <em>not</em> thread safe. If you have a multi threaded application, then the suggested use is to have one instance per thread.
 * </p>
 */
public class MimeTokenStream implements EntityStates, RecursionMode {
	 ...
	 * 
	 * @return <code>MimeTokenStream</code> which throws a <code>MimeException</code> whenever possible issues are dedicated in the input
	 */
	...
	/**
	 * Constructs a standard (lax) stream. Optional validation events will be logged only. Use {@link #createStrictValidationStream()} to create a stream that strictly validates the input.
	 */
	...
	/**
	 * Instructs the {@code MimeTokenStream} to parse the given streams contents. If the {@code MimeTokenStream} has already been in use, resets the streams internal state.
	 */
	...
	private void doParse(InputStream stream, String contentType) {
		...
		inbuffer = new BufferedLineReaderInputStream(stream, 4 * 1024, config.getMaxLineLen());
		...
		case M_RECURSE:
			MimeEntity mimeentity = new MimeEntity(lineSource, inbuffer, null, T_START_MESSAGE, T_END_MESSAGE, config);
	 ...
	 * 
	 * @return <code>true</code> if in raw mode, <code>false</code> otherwise.
	...
	/**
	 * Gets the current recursion mode. The recursion mode specifies the approach taken to parsing parts. {@link #M_RAW} mode does not parse the part at all. {@link #M_RECURSE} mode recursively parses each mail when an <code>message/rfc822</code> part is encounted; {@link #M_NO_RECURSE} does not.
	 * 
	...
	/**
	 * Sets the current recursion. The recursion mode specifies the approach taken to parsing parts. {@link #M_RAW} mode does not parse the part at all. {@link #M_RECURSE} mode recursively parses each mail when an <code>message/rfc822</code> part is encounted; {@link #M_NO_RECURSE} does not.
	 * 
	 * @param mode
	 *            {@link #M_RECURSE}, {@link #M_RAW} or {@link #M_NO_RECURSE}
	 */
	 ...
	 * <p/>
	 * This method is valid, if {@link #getState()} returns either of {@link #T_RAW_ENTITY}, {@link #T_PREAMBLE}, or {@link #T_EPILOGUE}.
	 * 
	 * @return Data stream, depending on the current state.
	 * @throws IllegalStateException
	 *             {@link #getState()} returns an invalid value.
	 */
	 ...
	 *             if there is no JVM support for decoding the charset
	 * @throws IllegalCharsetNameException
	 *             if the charset name specified in the mime type is illegal
	 */
	 ...
	 * @throws IllegalStateException
	 *             {@link #getState()} returns another value than {@link #T_FIELD}.
	 */
	 ...
	 * 
	 * @throws IllegalStateException
	 *             The method has been called, although {@link #getState()} was already {@link #T_END_OF_STREAM}.
	 */
