...
public final class MediaType extends Metadata {
	...
	/**
	 * The known media types registered with {@link #register(String, String)}, retrievable using {@link #valueOf(String)}.<br>
	 * Keep the underscore for the ordering.
	 */
	...
	/**
	 * {@inheritDoc}<br>
	 * In case the media type has parameters, this method returns the concatenation of the main type and the subtype. If the subtype is not equal to "*", it returns the concatenation of the main type and "*". Otherwise, it returns either the {@link #ALL} media type if it is already the {@link #ALL} media type, or null.
	 */
	@Override
	...
	@Override
	public int hashCode() {
		return SysUtils.hashCode(super.hashCode(), getParameters());
	}

	/**
	 * Indicates if a given media type is included in the current one @see {@link #includes(Metadata, boolean)}. It ignores the parameters.
	 * 
	 * @param included
