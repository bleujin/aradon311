...
public class NettyWebServer extends Radon {
	...
	public NettyWebServer(RadonConfiguration config) {
		...
		this.rootContext = config.getServiceContext() ;
	}
	...
	public synchronized Future<Radon> start() {
		FutureTask<Radon> future = new FutureTask<Radon>(new Callable<Radon>() {
			...
			public Radon call() throws Exception {
				...
				logger.warn("PORT " +  getConfig().getPort()) ;
				...
				bootstrap.setPipelineFactory(new ChannelPipelineFactory() {
					...
					public ChannelPipeline getPipeline() throws Exception {
						...
						pipeline.addLast("handler", new NettyHttpChannelHandler(rootContext, getConfig().executor(), getConfig().handlers(), id, timestamp, getConfig().exceptionHandler(), getConfig().ioExceptionHandler()));
						return pipeline;
					}
				});
