...
public class ResteasyProviderFactory extends RuntimeDelegate implements Providers {
	...
	protected static class SortedKey<T> implements Comparable<SortedKey<T>>, MediaTypeMap.Typed {
		...
		private SortedKey(Class intf, T reader, Class readerClass) {
			...
			/*
			 * Type impl = readerClass.getGenericSuperclass(); // if it's null or object, check the interfaces // TODO: we may need more refinement here. if (impl == null || impl == Object.class) { Type[] impls = readerClass.getGenericInterfaces(); if (impls.length > 0) { impl = impls[0]; } }
			 * 
			 * if (impl != null && (impl instanceof ParameterizedType)) { ParameterizedType param = (ParameterizedType) impl; if (param.getActualTypeArguments()[0].equals(Object.class)) isGeneric = true; } else { isGeneric = true; }
			 */
		}
	...
	/**
	 * Add a {@link ClientErrorInterceptor} to this provider factory instance. Duplicate handlers are ignored. (For Client Proxy API only)
	 */
	...
	public StringConverter getStringConverter(Class<?> clazz) {
		if (stringConverters.size() == 0)
			return null;
		return stringConverters.get(clazz);
	}

	public <T> StringParameterUnmarshaller<T> createStringParameterUnmarshaller(Class<T> clazz) {
		if (stringParameterUnmarshallers.size() == 0)
			return null;
...
//	private static List<MessageBodyWriter> mbws = ListUtil.<MessageBodyWriter>toList(new ByteArrayProvider(), new StreamingOutputProvider(), new InputStreamProvider(), new IIOImageProvider(), new FileProvider(), new FormUrlEncodedProvider(), new StringTextStar(), new DefaultTextPlain()) ; 
//	public <T> MessageBodyWriter<T> getMessageBodyWriter(Class<T> type, Type genericType, Annotation[] annotations, MediaType mediaType) {
//		for (MessageBodyWriter writer : mbws) {
//			if (writer.isWriteable(type, genericType, annotations, mediaType)) {
//				return (MessageBodyWriter<T>) writer;
//			}
//		}
//		return (MessageBodyWriter<T>)new DefaultTextPlain();
//	}
