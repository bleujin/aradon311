...
public class ClientRequest extends ClientInterceptorRepositoryImpl implements Cloneable {
	...
	public ClientRequest(UriBuilder uri, ClientExecutor executor, ResteasyProviderFactory providerFactory) {
		...
		if (providerFactory instanceof ProviderFactoryDelegate) {
			this.providerFactory = ((ProviderFactoryDelegate) providerFactory).getDelegate();
		} else {
	...
	public ClientResponse execute() throws Exception {
		...
		if (getReaderInterceptorList().isEmpty())
			setReaderInterceptors(providerFactory.getClientMessageBodyReaderInterceptorRegistry().bindForList(null, null));

		if (getExecutionInterceptorList().isEmpty()) {
			setExecutionInterceptors(providerFactory.getClientExecutionInterceptorRegistry().bindForList(null, null));
		}
		...
		if (getExecutionInterceptorList().isEmpty()) {
			...
			ClientExecutionContextImpl ctx = new ClientExecutionContextImpl(getExecutionInterceptorList(), executor, this);
	...
	public void writeRequestBody(MultivaluedMap<String, Object> headers, OutputStream outputStream) throws IOException {
		...
		if (getWriterInterceptorList().isEmpty()) {
			setWriterInterceptors(providerFactory.getClientMessageBodyWriterInterceptorRegistry().bindForList(null, null));
		}
		MessageBodyWriter writer = providerFactory.getMessageBodyWriter(bodyType, bodyGenericType, bodyAnnotations, bodyContentType);
		if (writer == null) {
			throw new RuntimeException("could not find writer for content-type " + bodyContentType + " type: " + bodyType.getName());
		}
		new ClientMessageBodyWriterContext(getWriterInterceptors(), writer, body, bodyType, bodyGenericType, bodyAnnotations, bodyContentType, headers, outputStream, attributes).proceed();
	}
