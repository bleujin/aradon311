...
public class MultipartInputImpl implements MultipartInput {
	...
	public MultipartInputImpl(MediaType contentType, Providers workers) {
		...
		if (httpRequest != null) {
			String defaultContentType = (String) httpRequest.getAttribute(InputPart.DEFAULT_CONTENT_TYPE_PROPERTY);
			
			if (defaultContentType != null)
				this.defaultPartContentType = MediaType.valueOf(defaultContentType);
	...
	public class PartImpl implements InputPart {
		...
		public PartImpl(BodyPart bodyPart) {
			...
			if (getCharset(contentType) == null) {
				if (defaultPartCharset != null) {
					...
					contentType = getMediaTypeWithCharset(contentType, "utf-8");
				}
			}
		}
		...
		public InputStream getBody() throws IOException {
			...
			if (body instanceof TextBody) {
				...
				/*
				 * InputStreamReader reader = (InputStreamReader)((TextBody) body).getReader(); StringBuilder inputBuilder = new StringBuilder(); char[] buffer = new char[1024]; while (true) { int readCount = reader.read(buffer); if (readCount < 0) { break; } inputBuilder.append(buffer, 0, readCount); } String str = inputBuilder.toString(); return new
