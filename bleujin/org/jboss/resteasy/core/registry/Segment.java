...
public class Segment {
	...
	/**
	 * @param httpMethod
	 *            this is so we can find a GET match when HEAD is called (and there is no head)
	 * @param request
	 * @return
	 */
	protected ResourceInvoker match(String httpMethod, HttpRequest request) {
		...
		/*
		 * I'm keeping this code around as its a different way to match. It takes producer preference over picking accept type listed first.
		 * 
		 * 
		 ...
		 * 
		 * WeightedMediaType last = null; List<WeightedMediaType> level = null; for (WeightedMediaType accept : accepts) { if (last == null || accept.getWeight() < last.getWeight()) { level = new ArrayList<WeightedMediaType>(); acceptsLevels.add(level); level.add(accept);
		 * 
		 * } else { level.add(accept); } last = accept; }
		 * 
		 * // preferred produces are matched first with preferred accepts. for (List<WeightedMediaType> acceptLevel : acceptsLevels) { for (WeightedMediaType produce : produces) { for (WeightedMediaType accept : acceptLevel) { if (accept.isCompatible(produce)) { // this is a big hack to propagate chosen accept type to ServerResponse request.setAttribute(RESTEASY_CHOSEN_ACCEPT, accept); return
		 * producesMap.get(produce); } } } }
		 */
