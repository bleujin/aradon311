...
public class ServerResponse extends Response {
	...
	public void writeTo(HttpRequest request, HttpResponse response, ResteasyProviderFactory providerFactory) throws WriterException {
		...
		MessageBodyWriter writer = providerFactory.getMessageBodyWriter(type, generic, annotations, contentType);
		...
		try {
			...
			if (size > -1)
				getMetadata().putSingle(HttpHeaderNames.CONTENT_LENGTH, String.valueOf(size));
	...
	public void commitHeaders(HttpResponse response) {
		if (getMetadata() != null) {
			List<Object> cookies = getMetadata().get(HttpHeaderNames.SET_COOKIE);
			if (cookies != null) {
